...
namespace SucCs.BitVector
{
    ...
    /// </para>
    /// </remarks>
    public class BitIndex<T> : IBitIndex
         where T : IIndexAccessibleCollection
    {
        ...
        private List<byte> _blockTable = new List<byte>(0);

        private ulong _length = 0;
        private ulong _totalPopCount = 0;

        public BitIndex(IIndexAccessibleCollection data, bool buildIndex = false)
        {
            _data = data;
            if (buildIndex)
                BuildIndex();
        ...
        public ulong Select(bool value, ulong rank)
        {
            ...
            // in this case, each table have just 4block, so we won't use binary search.
            var i = 1;
            var min = (nextBlockStart > this._blockTable.Count) ? this._blockTable.Count : nextBlockStart;
            for (; ++blockIndex < min; i++)
            {
                var blockRank = (ulong)(value ? this._blockTable[blockIndex] : (i * BLOCK_SIZE) - this._blockTable[blockIndex]);
                if (rank <= blockRank)
                    break; // this block contains enough popcount.
                //i++;
            }
            rank -= (ulong)(value ? this._blockTable[--blockIndex] : (--i * BLOCK_SIZE) - this._blockTable[--blockIndex]);
        ...
        public void BuildIndex()
        {
            ...
            try
            {
                checked
                {
                    ...
                    this._blockTable.AddRange(new byte[blockCount]);
            ...
            try
            {
                checked
                {
                    for (var i = 0; i < this._rankTable.Count; i++)
                    {
                        ...
                        {
                            subTotal += (byte)BitUtility.PopCountInt64(this._data.GetRangeInt64((ulong)j * BLOCK_SIZE));
